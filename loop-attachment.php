...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				...
				<?php if ( ! empty( $post->post_parent ) ) : ?>
					<p class="page-title"><a href="<?php echo get_permalink( $post->post_parent ); ?>" title="<?php esc_attr( printf( __( 'Return to %s', 'wp_distinctionpp' ), get_the_title( $post->post_parent ) ) ); ?>" rel="gallery"><?php
						/* translators: %s - title of parent post */
						printf( __( '<span class="meta-nav">&larr;</span> %s', 'wp_distinctionpp' ), get_the_title( $post->post_parent ) );
					?></a></p>
				<?php endif; ?>
...
?>
						<span class="attachment"><a href="<?php echo $next_attachment_url; ?>" title="<?php echo esc_attr( get_the_title() ); ?>" rel="attachment"><?php
							$attachment_width  = apply_filters( 'wp_distinctionpp_attachment_size', 868 );
							$attachment_height = apply_filters( 'wp_distinctionpp_attachment_height', 868 );
